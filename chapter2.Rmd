...
drop1(my_model2)
```
These analysis suggest that we should drop 'surf' from the model as a non-influencial variable. Let's keep 'stra' still in the game because it's AIC value difference to the next best model is so small that it probably is also influencial althoug attitude clearly seems to be the most important variable.

Let's fit a new model with only attitude and stra:
```{r}
...
Next there is a coefficient table. Regression coefficients represent the mean change in the response variable (attitude) for one unit of change in the predictor variable (attitude and stra) while holding other predictors in the model constant.

The p-value for each term tests the null hypothesis that the coefficient is equal to zero (no effect). Strategic learning approach is not anymore statistically significant but I want to keep it in the model because AIC values showed that it is very close to being significant.

From these results we can deduce that attitude has a strong positive effect and strategic learning approach a slightly weaker positive effect on points.
...
In this case everything seems to be ok. There are few values (numbered) that have lower values but they are not clear outliers on my opinion. 
...
##Conclusions
My model seems to be a valid one. It explains around 20 % of the variation in the points that students scored in an exam. The most important factors affecting positively the points were attitude and strategic learning approach. 
